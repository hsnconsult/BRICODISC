...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_bypass_source_scan(self):
        ...
        lot1 = self.env['stock.production.lot'].create(
            {'name': 'lot1', 'product_id': self.productlot1.id, 'company_id': self.env.company.id})
        lot2 = self.env['stock.production.lot'].create(
            {'name': 'serial1', 'product_id': self.productserial1.id, 'company_id': self.env.company.id})
    ...
    def test_picking_owner_scan_package(self):
        ...
        self.env['stock.quant']._update_available_quantity(self.product1, self.stock_location, 7,
                                                           package_id=self.package, owner_id=self.owner)
        action_id = self.env.ref('stock_barcode.stock_barcode_action_main_menu')
        url = "/web#action=" + str(action_id.id)
...
class TestInventoryAdjustmentBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_inventory_adjustment_mutli_location(self):
        ...
        inventory = self.env['stock.inventory.line'].search([('product_id', '=', self.product1.id)],
                                                            limit=1).inventory_id
        self.assertTrue(inventory)
    ...
    def test_inventory_adjustment_tracked_product(self):
        ...
        inventory = self.env['stock.inventory.line'].search([('product_id', '=', self.productlot1.id)],
                                                            limit=1).inventory_id
        self.assertTrue(inventory)
        self.assertEqual(len(inventory.line_ids), 4)
