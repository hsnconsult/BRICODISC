...
class Company(models.Model):
    ...
    journalcaisse_pos = fields.Many2one('account.journal', string='Journal caisse')
    journalbanque_pos = fields.Many2one('account.journal', string='Journal banque')
...
class PosSession(models.Model):
    ...
    def custom_bank(self):
        ...
        vals1 = {
            ...
            'name': 'Transfert banque',
            'debit': 0,
            'credit': self.total_payments_amount - self.cash_register_total_entry_encoding,
        }
        aml.append((0, False, vals1))
        vals2 = {
            'account_id': company.journalbanque_pos.default_credit_account_id.id,
            'name': 'Transfert banque',
            'debit': self.total_payments_amount - self.cash_register_total_entry_encoding,
            'credit': 0,
        }
        aml.append((0, False, vals2))

        amc = am.create(
            {'journal_id': company.journalbanque_pos.id, 'date': self.start_at + timedelta(days=1), 'ref': self.name})
        for record in amc:
            record.write({'line_ids': aml})
            record.post()
        # self.write({'remis': True})
