...
class IwelReception(models.Model):
    ...
    def see_receptions(self):
        action = self.env.ref('stock.action_picking_tree_all').read()[0]
        receptions = self.env['stock.picking'].search([('origin', 'in', [x.name for x in self.po_ids])])
        if len(receptions) > 0:
            action['domain'] = [('origin', 'in', [x.name for x in self.po_ids])]
    ...
    def _prepare_purchase_order_data(self, reception):
        ...
        if reception:
            ...
            data = {
                ...
                'date_order': reception.datereception if reception.datereception else fields.datetime.now(),
                'date_planned': reception.datereception if reception.datereception else fields.datetime.now(),
                'state': 'draft',
