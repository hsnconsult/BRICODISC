...
class iwel_ligneetiquette(models.Model):
    ...
    def barcode_onchange(self):
        self.nomp = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].name if self.env[
            'product.template'].search([('barcode', '=', self.barcode)]) else ""
        self.codep = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].default_code if self.env[
            'product.template'].search([('barcode', '=', self.barcode)]) else ""
        self.codef = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].c4 if self.env[
            'product.template'].search([('barcode', '=', self.barcode)]) else ""
        self.date = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].c1 if self.env[
            'product.template'].search([('barcode', '=', self.barcode)]) else ""
        self.prix = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].list_price if self.env[
            'product.template'].search([('barcode', '=', self.barcode)]) else ""
        self.currency_id = self.env['product.template'].search([('barcode', '=', self.barcode)])[0].currency_id.id if \
            self.env['product.template'].search([('barcode', '=', self.barcode)]) else ""
    ...
    def get_name(self):
        for r in self:
            r.name = "Ligne etiquette %s" % s(r.id)
